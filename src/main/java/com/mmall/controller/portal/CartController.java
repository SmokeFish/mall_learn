...
public class CartController {
    ...
    @ResponseBody
    public ServerResponse selectProject(HttpSession session,Integer productId){
        User user = (User) session.getAttribute(Const.CURRENT_USER);
        if(user == null){
            return ServerResponse.createByErrorCodeMessage(ResponseCode.NEED_LOGIN.getCode(),ResponseCode.NEED_LOGIN.getDesc());
        }
        return iCartService.selectProject(user.getId(),productId);
    }
    @RequestMapping("un_select.do")
    @ResponseBody
    public ServerResponse unSelectProject(HttpSession session,Integer productId){
        User user = (User) session.getAttribute(Const.CURRENT_USER);
        if(user == null){
            return ServerResponse.createByErrorCodeMessage(ResponseCode.NEED_LOGIN.getCode(),ResponseCode.NEED_LOGIN.getDesc());
        }
        return iCartService.unSelectProject(user.getId(),productId);
    }
